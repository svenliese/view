...
public class CardInfo<C> {
    ...
    private final SimpleText<C> leadInfo;
    ...
    public CardInfo(KanbanModel model, Column column, ViewBounds viewBounds, IColorFactory<C> colorFactory, float cardSize) {
        ...
        if(column.getTypeId().equals(KanbanModel.TYPE_DONE)) {
            ...
            leadInfo = new SimpleText<>(getLeadInfoText(), colorFactory.getWhite());
            leadInfo.setXPercentage(viewBounds.getX() + 0.01f);
            leadInfo.setYPercentage(y);
            leadInfo.setWPercentage(viewBounds.getW() - 0.02f);
            leadInfo.setHPercentage(KanbanCompareModelView.textHeight);
            leadInfo.setTextSize(KanbanCompareModelView.textSize);
            allViews.add(leadInfo);
        } else {
            timeInfo = null;
            leadInfo = null;
    ...
    private String getLeadInfoText() {
        return "lead "+model.getLeadTime() / Interval.MILLIS_PER_HOUR;
    }
    ...
    public void update() {
        ...
        if(column.getTypeId().equals(KanbanModel.TYPE_DONE)) {
            ...
            leadInfo.setText(getLeadInfoText());
        } else {
